...
- A completed host fallback requires a provider-native session identity. Missing identity returns `unavailable`; Review Loop never fabricates independence.
...
| Existing Agent Kernel v2 transaction | Continues to use authorized `--tier` path without internal fallback | AK-CUTOVER replaces it with Kernel-owned request/transaction v3 |
